module Mailgun
  ...
  class MailingList::Member
    ...
    # Update set of mailing list members with csv file
    # First Last, user@domain.org
    def update_csv(csv, options={})
      params = { :members => csv, upsert: true, ignore_errors: true, subscribed: true }
      Mailgun.submit :post, "#{@mailgun.base_url}/lists#{'/' + @address}/members.csv", params.merge(options)
    end
