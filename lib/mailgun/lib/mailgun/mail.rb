...
module Mailgun
  class Mail
    ...
    def send(options)
      raise ArgumentError.new("Message must have a sender and at least one recipient: #{options.inspect}") if !options.has_key?(:from) || !(options.has_key?(:to) || options.has_key?(:cc) || options.has_key?(:bcc))
